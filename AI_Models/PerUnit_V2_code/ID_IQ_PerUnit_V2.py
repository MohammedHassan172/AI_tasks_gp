...
joblib.dump(input_scaler, 'input_scaler__ID&IQ_PerUnit.pkl')
...
# Infinite testing loop with per-unit handling
try:
    # Get the rated values for scaling
    rated_Iq = float(input("Enter the rated Iq value: "))
    rated_Id = float(input("Enter the rated Id value: "))
    ...
    while True:
        ...
        new_input_per_unit = new_input / np.array([[rated_Iq, rated_Id]])

        # Standardize the per-unit input using the input scaler
        new_input_scaled = input_scaler.transform(new_input_per_unit)
